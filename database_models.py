...
class OMRSheet(Base):
    ...
    processing_status = Column(String, default="uploaded")  # uploaded, processing, completed, error
...
class ExamConfig(Base):
    ...
    subjects = Column(Text)  # JSON string of subject list
    questions_per_subject = Column(Integer, default=20)
    answer_key = Column(Text)  # JSON string of answers
...
class Result(Base):
    ...
    detected_answers = Column(Text)  # JSON string of detected answers
...
class ProcessingLog(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    sheet_id = Column(Integer)
    stage = Column(String)  # upload, preprocessing, detection, scoring, completed
